...
非 `/` 消息由 **unified_chat** 一次调用完成判断 + 回复，不再单独分类：

```
"帮我写首歌"   → AI 判断需调用工具 → [TOOL:music]    → 音乐插件
"今天热不热"   → AI 判断需调用工具 → [TOOL:weather]  → 天气插件
"你可以创作歌曲吗？" → AI 判断是能力询问 → 直接回复 "可以的！"
"你好啊"      → AI 判断是闲聊      → 直接回复
```

- 询问能力（"你能…吗？"）→ 直接对话，不触发插件
- 明确请求（"帮我…"）→ AI 输出 `[TOOL:xxx]` 标记 → 路由到对应插件
- `/` 命令直通插件，零延迟
...
全部 AI 调用使用 **MiniMax M2.7**，无需本地 GPU——unified_chat（对话 + 意图分发合一）、每日总结、音乐歌词、搜索结果总结。`.env` 配置 API Key 即可。
